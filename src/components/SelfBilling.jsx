...
export default function SelfBilling() {
  ...
  const handleStart = async () => {
    try {
      const response = await fetch("https://salonik07-autobill-backend.hf.space/start");
 ...
 const handleStop = async () => {
  try {
    const response = await fetch("https://salonik07-autobill-backend.hf.space/stop", {
      method: 'POST',  
      headers: {
...
useEffect(() => {
  const stopOnUnload = () => {
    if (detectionStarted) {
      navigator.sendBeacon("https://salonik07-autobill-backend.hf.space/stop");
    }
  };
